...
-----------------------
Analyzes sales and inventory data to identify imbalances and optimization opportunities.
"""
...
class InventoryAnalyzer:
    ...
    def analyze_sales_data(self):
        ...
        sales_metrics = self.sales_df.groupby(['store_id', 'product_id']).agg({
            'quantity': ['sum', 'mean', 'std', 'count'],
            'revenue': ['sum', 'mean']
        }).reset_index()

        # Flatten the multi-level columns
        sales_metrics.columns = ['_'.join(col).strip('_') for col in sales_metrics.columns.values]

        # Calculate coefficient of variation (CV) to measure sales consistency
        sales_metrics['quantity_cv'] = sales_metrics['quantity_std'] / sales_metrics['quantity_mean']
        sales_metrics['quantity_cv'].fillna(0, inplace=True)

        # Calculate days since last sale
        last_sale_date = self.sales_df.groupby(['store_id', 'product_id'])['date'].max().reset_index()
        last_sale_date.columns = ['store_id', 'product_id', 'last_sale_date']

        end_date = self.sales_df['date'].max()
        last_sale_date['days_since_last_sale'] = (end_date - last_sale_date['last_sale_date']).dt.days

        # Merge metrics with inventory data
        analysis_df = pd.merge(self.inventory_df, sales_metrics, on=['store_id', 'product_id'], how='left')
        analysis_df = pd.merge(analysis_df, last_sale_date, on=['store_id', 'product_id'], how='left')

        # Fill NaN values for products with no sales
        for col in ['quantity_sum', 'quantity_mean', 'quantity_std', 'quantity_count',
                   'revenue_sum', 'revenue_mean', 'quantity_cv']:
            analysis_df[col].fillna(0, inplace=True)

        # Calculate days of inventory based on average daily sales
        # Use 365 days to convert total sales to daily average
        days_in_data = (self.sales_df['date'].max() - self.sales_df['date'].min()).days + 1
        analysis_df['avg_daily_sales'] = analysis_df['quantity_sum'] / days_in_data
        analysis_df['avg_daily_sales'].replace(0, 0.01, inplace=True)  # Avoid division by zero

        analysis_df['days_of_inventory'] = analysis_df['current_stock'] / analysis_df['avg_daily_sales']
        ...
        if isinstance(self.products, pd.DataFrame):
            product_category_map = self.products.set_index('product_id')['category'].to_dict()
            analysis_df['category'] = analysis_df['product_id'].map(product_category_map)
    ...
    def identify_inventory_imbalances(self, min_days=7, max_days=30):
        """
        ...
        excess_mask = (self.analysis_df['days_of_inventory'] > max_days) & (self.analysis_df['current_stock'] > 0)
        ...
        self.analysis_df['excess_units'] = np.where(
            excess_mask,
            self.analysis_df['current_stock'] - (max_days * self.analysis_df['avg_daily_sales']).astype(int),
            0
        )
        ...
        self.analysis_df['needed_units'] = np.where(
            need_mask,
            ((min_days * self.analysis_df['avg_daily_sales']) - self.analysis_df['current_stock']).astype(int),
            0
        )
        ...
        print(f"- Excess inventory items: {excess_count} (total units: {total_excess_units})")
        print(f"- Needed inventory items: {needed_count} (total units: {total_needed_units})")
    ...
    def evaluate_plan_impact(self, transfer_plan):
        ...
        for _, transfer in transfer_plan.iterrows():
            ...
            from_mask = (post_inventory['store_id'] == from_store_id) & (post_inventory['product_id'] == product_id)
            post_inventory.loc[from_mask, 'current_stock'] -= units

            # Update inventory at destination store
            to_mask = (post_inventory['store_id'] == to_store_id) & (post_inventory['product_id'] == product_id)
        ...
        # Calculate days of inventory based on average daily sales
        post_analysis['days_of_inventory'] = post_analysis['current_stock'] / post_analysis['avg_daily_sales']
        post_analysis['days_of_inventory'].replace(np.inf, 365, inplace=True)  # Cap at 1 year for zero sales
        ...
        # Identify excess inventory (more than max_days of stock)
        excess_mask = (post_analysis['days_of_inventory'] > max_days) & (post_analysis['current_stock'] > 0)
        ...
        total_cost = transfer_plan['transport_cost'].sum() if 'transport_cost' in transfer_plan.columns else 0
        ...
        turnover_improvement = (turnover_after - turnover_before) / turnover_before * 100 if turnover_before > 0 else 0

        # Calculate inventory balance improvement
        imbalance_before = std_days_before
        imbalance_after = std_days_after
        balance_improvement = (imbalance_before - imbalance_after) / imbalance_before * 100 if imbalance_before > 0 else 0
        ...
        if isinstance(self.products, pd.DataFrame) and 'cost' in self.products.columns:
            ...
            post_analysis['product_value'] = post_analysis['product_id'].map(product_value_map)

            # Calculate inventory value before and after
            inventory_value_before = (self.analysis_df['current_stock'] *
                                    self.analysis_df['product_id'].map(product_value_map)).sum()
            inventory_value_after = (post_analysis['current_stock'] * post_analysis['product_value']).sum()

            # Calculate excess inventory value before and after
            excess_value_before = (self.analysis_df.loc[self.analysis_df['inventory_status'] == 'Excess', 'current_stock'] *
                                self.analysis_df.loc[self.analysis_df['inventory_status'] == 'Excess', 'product_id'].map(product_value_map)).sum()

            excess_value_after = (post_analysis.loc[post_analysis['post_inventory_status'] == 'Excess', 'current_stock'] *
                                post_analysis.loc[post_analysis['post_inventory_status'] == 'Excess', 'product_value']).sum()
        else:
            ...
            excess_value_before = self.analysis_df.loc[self.analysis_df['inventory_status'] == 'Excess', 'current_stock'].sum()
            excess_value_after = post_analysis.loc[post_analysis['post_inventory_status'] == 'Excess', 'current_stock'].sum()
        ...
        impact_summary = {
            ...
            'Improvement': {
                'Reduction in Excess Items': before_counts.get('Excess', 0) - after_counts.get('Excess', 0),
                'Reduction in Needed Items': before_counts.get('Needed', 0) - after_counts.get('Needed', 0),
                'Increase in Balanced Items': after_counts.get('Balanced', 0) - before_counts.get('Balanced', 0),
...
if __name__ == "__main__":
    ...
    analyzer.load_data(
        ...
        stores_path=os.path.join(data_dir, "stores.csv") if os.path.exists(os.path.join(data_dir, "stores.csv")) else None,
        products_path=os.path.join(data_dir, "products.csv") if os.path.exists(os.path.join(data_dir, "products.csv")) else None
    )
    ...
    if 'category' in analysis_df.columns:
        ...
        category_status = analysis_df.groupby(['category', 'inventory_status']).size().unstack().fillna(0)
        print(category_status)

    if 'city' in analysis_df.columns:
        print("\nInventory Status by City:")
        city_status = analysis_df.groupby(['city', 'inventory_status']).size().unstack().fillna(0)
        print(city_status)
